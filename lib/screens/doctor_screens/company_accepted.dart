...
import 'package:hospital_management_system/screens/bottom_nav_company.dart';
...
class CompanyAcceptedPage extends StatefulWidget {
  const CompanyAcceptedPage({Key? key}) : super(key: key);

  @override
  State<CompanyAcceptedPage> createState() => _CompanyAcceptedPageState();
}

class _CompanyAcceptedPageState extends State<CompanyAcceptedPage> {
  @override
            ...
            child: Column(
              ...
              children: [
                ...
                FutureBuilder<List<GetCompanyUIAppointment?>>(
                  future:
                      CategoryRepository().getCompanyUIAppointment("Accepted"),
  ...
  endAppointment(GetCompanyUIAppointment appointmentResponse) async {
    // debugPrint("Appointment ID: " + appointmentResponse.id!);
    bool isAccepted = await CategoryRepository()
        .updateCompanyUIAppointment(appointmentResponse.id!, "Ended");
    if (isAccepted) {
      ...
      Future.delayed(
        ...
        () {
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
                builder: (context) => const BottomNavCompany(index: 1)),
          );
        },
      );
    } else {
